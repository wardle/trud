       ...
       (let [buffer (make-array Byte/TYPE buffer-size)
         ...
         (loop [count 0 total 0]
           (let [size (.read input buffer)]
             (if-not (pos? size)
               (when show-progress? (pr/print (pr/tick progress total)))
...
(defn- download-archive-file-to-cache
  ...
  (let [cp (cache-path dir release)]
    ...
    (println "Downloading TRUD item:" itemIdentifier)
    (clojure.pprint/pprint (select-keys release [:itemIdentifier :releaseDate :archiveFileName archiveFileSizeBytes]))
    (try
      (download-url archiveFileUrl cp {:show-progress? true :expected-size-bytes archiveFileSizeBytes})
      (validate-file cp release)
